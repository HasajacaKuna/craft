...
// Duży kafelek: zdjęcie + NAZWA + CENA (większe, jak prosisz)
function BigTileCard({ product }: { product: Product }) {
  ...
  return (
    <a href={`/produkt/${product.slug || product._id}`} className="group block">
      <div className="overflow-hidden rounded-3xl bg-gray-100 shadow-sm dark:bg-gray-800">
        ...
        {product.image ? (
          <img
            src={product.image}
            alt={title}
            className="h-full w-full object-cover transition duration-300 group-hover:scale-[1.02] aspect-[3/4] md:aspect-[4/5]"
            loading="lazy"
          />
        ) : (
          <div className="aspect-[3/4] md:aspect-[4/5] flex items-center justify-center text-sm text-gray-500">Brak zdjęcia</div>
        )}
      </div>
...
function ProductCarousel({ title = 'Wybierz coś dla siebie', products = [] as Product[] }) {
  const MAX_ITEMS = 12
...
<Section id="produkty" title="">
  ...
  <div className="mb-10 text-center">
    <h2 className="text-3xl font-bold md:text-4xl">Nowości</h2>
    <div className="mx-auto mt-3 h-[2px] w-16 bg-gray-300" />   {/* kreska wycentrowana */}
    ...
    >
      <ChevronRight className="w-4 h-4 sm:w-8 sm:h-8" />
        ...
        <div className="mx-auto max-w-md rounded-2xl border p-6 text-center dark:border-gray-800">
          ...
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">Dodaj paski, aby wyświetlić je na stronie głównej.</p>
        </div>
      )}
...
export default function HomePage() {
  const allProducts = categories.flatMap((c: any) => c.products) as Product[]
